...
class base_Env:
    def __init__(self, args, load_ftn):
        ...
        self.env = args.env
        self.args = args
        #TODO below is not needed for Wine dataset
        ...
        #TODO for wine integration, remove auto encoder
        ...
        for Bidx in range(self.N0 // B + 1):
            ...
            #TODO for wine integration
            #X0_stack.append(self.X0[Bidx * B: (Bidx + 1) * B, :]) #remove encoding
        X1_stack = []
        for Bidx in range(self.N1 // B + 1):
            X1_stack.append(self.encoding(self.X1[Bidx * B: (Bidx + 1) * B, :]).copy())
            #TODO for wine integration
            #X1_stack.append(self.X1[Bidx * B: (Bidx + 1) * B, :]) #remove encoding
