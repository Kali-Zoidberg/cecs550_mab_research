"""
Algorithm Code from paper
"Contextual Linear Bandits under Noisy Features: Towards Bayesian Oracles"
https://arxiv.org/pdf/1703.01347
https://github.com/junghunkim7786/contextual_linear_bandits_under_noisy_features/blob/aistats/algorithms.py#L4
"""
...
def _CLBEF_get_UCB(x_hat, t, K, theta_hat, d, T, V_inv, p_hat, x_sum):
    return x_hat @ theta_hat + (np.sqrt((d + 1) * np.log((t + 2) * T)) + x_sum * d * ((1 / p_hat) ** (3 / 2)) * np.sqrt(
        np.log(T) * np.log(K * T) / ((t + 2) * K)) + \
...
def _CLBEF_x_bars(nu_hat, Sigma_hat, x, m, x_bar_dummy):
    ...
    if len(index_S) > 0:
        ...
        if len(index_U) > 0:
            x_U = nu_hat[index_U] + numba_ix(Sigma_hat, index_U, index_S) @ np.linalg.pinv(
                numba_ix(Sigma_hat, index_S, index_S)) @ (x_S - nu_hat[index_S]).T
...
def _CLBEF_get_estimators(d, K, x_t, m_t, Kt, xi, n, Z):
    ...
    Sigma_hat = Z * (((p_hat - 1) / (p_hat * p_hat)) * np.identity(d) + 1 / (p_hat * p_hat)) / (Kt) - np.outer(nu_hat,
                                                                                                               nu_hat)
...
class CLBBF:
    ...
    def run(self):
        ...
        for t in tqdm(range(self.T)):
            ...
            self.p_hat, self.nu_hat, self.Sigma_hat = _CLBEF_get_estimators(self.d, self.K, x_t, m_t, self.K * (t + 1),
                                                                            self.xi, self.n, self.Z)
            ...
            if t == 0:
                ...
                chosen_arm, max_ucb = _CLBEF_UCB(self.x_hat, t, self.K, self.theta_hat, self.d, self.T, self.V_inv,
                                                 self.p_hat, self.x_sum)
